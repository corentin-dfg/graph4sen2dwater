...
class Graph(pyg.data.HeteroData):
    ...
    def __init_grid_node_features(self):
        ...
        ymd_to_day_of_year_func = lambda dateYMD : datetime(*dateYMD).timestamp() / (3600*24*365.24219) % 1 # Normalization as in GraphCast
        day_of_year = torch.as_tensor(list(map(ymd_to_day_of_year_func, self.dates))).unsqueeze(0).expand(self['grid'].node_index.shape[0],-1)
        sin_date = torch.sin(day_of_year)
        cos_date = torch.cos(day_of_year)
        sin_lat = torch.sin(self['grid'].pos[:,(1,)].deg2rad())
        cos_lon = torch.cos(self['grid'].pos[:,(0,)].deg2rad())
        sin_lon = torch.sin(self['grid'].pos[:,(0,)].deg2rad())
        self['grid'].x = torch.cat((ndwi, sin_date, cos_date, sin_lat, cos_lon, sin_lon), dim=1)

    def __init_mesh_node_features(self):
        sin_lat = torch.sin(self['mesh'].pos[:,(1,)].deg2rad())
        cos_lon = torch.cos(self['mesh'].pos[:,(0,)].deg2rad())
        sin_lon = torch.sin(self['mesh'].pos[:,(0,)].deg2rad())
        self['mesh'].x = torch.cat((sin_lat, cos_lon, sin_lon), dim=1)

    def __init_grid2mesh_edge_features(self):
        pos_diff = self['grid'].pos[self['grid', 'to', 'mesh'].edge_index[0]] - self['mesh'].pos[self['grid', 'to', 'mesh'].edge_index[1]]
        length_edge = torch.sqrt(torch.sum(pos_diff**2,dim=1,keepdim=True))
        self['grid', 'to', 'mesh'].edge_attr = torch.cat((length_edge, pos_diff), dim=1)
        if self['grid', 'to', 'mesh'].edge_attr.numel() > 0:
            self['grid', 'to', 'mesh'].edge_attr = (self['grid', 'to', 'mesh'].edge_attr / self['grid','to','mesh'].edge_attr.amax(0)).nan_to_num()

    def __init_mesh2mesh_edge_features(self):
        pos_diff = self['mesh'].pos[self['mesh', 'to', 'mesh'].edge_index[0]] - self['mesh'].pos[self['mesh', 'to', 'mesh'].edge_index[1]]
        length_edge = torch.sqrt(torch.sum(pos_diff**2,dim=1,keepdim=True))
        self['mesh', 'to', 'mesh'].edge_attr = torch.cat((length_edge, pos_diff), dim=1)
        if self['mesh', 'to', 'mesh'].edge_attr.numel() > 0:
            self['mesh', 'to', 'mesh'].edge_attr = (self['mesh', 'to', 'mesh'].edge_attr / self['mesh','to','mesh'].edge_attr.amax(0)).nan_to_num()

    def __init_mesh2grid_edge_features(self):
        pos_diff = self['mesh'].pos[self['mesh', 'to', 'grid'].edge_index[0]] - self['grid'].pos[self['mesh', 'to', 'grid'].edge_index[1]]
        length_edge = torch.sqrt(torch.sum(pos_diff**2,dim=1,keepdim=True))
        self['mesh', 'to', 'grid'].edge_attr = torch.cat((length_edge, pos_diff), dim=1)
        if self['mesh', 'to', 'grid'].edge_attr.numel() > 0:
            self['mesh', 'to', 'grid'].edge_attr = (self['mesh', 'to', 'grid'].edge_attr / self['mesh','to','grid'].edge_attr.amax(0)).nan_to_num()
