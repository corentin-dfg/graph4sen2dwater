...
from data import SitsDataset, SitsToGraph, DeterminedSubSitsFromEnd
...
writer = SummaryWriter(comment=f"_{model._get_name()}_SEED-{SEED}_SPLX-{NB_SPXL}_COMPACTNESS-{COMPACTNESS}_TLEN-{T_LEN}")
...
for epoch in range(EPOCHS):
    ...
    for data in tqdm(train_dl, desc=f"[TRAIN {epoch+1}/{EPOCHS}]"):
        ...
        train_mse += mse(next_ndwi.reshape((-1, BANDS, IMG_HEIGHT, IMG_WIDTH)), data['grid'].y.reshape((-1, BANDS, IMG_HEIGHT, IMG_WIDTH))).item()
    ...
    writer.add_scalar('MSE/train', train_mse/len(train_dl), epoch+1)
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(val_dl, desc=f"[VAL {epoch+1}/{EPOCHS}]")):
            ...
            val_loss += loss.item()
            val_mse += mse(next_ndwi.reshape((-1, BANDS, IMG_HEIGHT, IMG_WIDTH)), data['grid'].y.reshape((-1, BANDS, IMG_HEIGHT, IMG_WIDTH))).item()
    ...
    writer.add_scalar('MSE/val', val_mse/len(val_dl), epoch+1)
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(test_dl, desc=f"[TEST {epoch+1}/{EPOCHS}]")):
            ...
            test_loss += loss.item()
            test_mse += mse(next_ndwi.reshape((-1, BANDS, IMG_HEIGHT, IMG_WIDTH)), data['grid'].y.reshape((-1, BANDS, IMG_HEIGHT, IMG_WIDTH))).item()
    ...
    writer.add_scalar('MSE/test', test_mse/len(test_dl), epoch+1)
