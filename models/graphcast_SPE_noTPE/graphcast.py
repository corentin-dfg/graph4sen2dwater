...
from data.graphdata_latlon_noTPE import Graph
...
class GraphCast(nn.Module):
    def __init__(self, n_features, t_len, hid_dim, n_processor_layers):
        ...
        self.grid2mesh = Grid2MeshGNN(hid_dim, hid_dim, hid_dim, hid_dim)
        processor_layers = []
        for i in range(self.n_processor_layers):
            processor_layers.append(Mesh2MeshGNN(hid_dim, hid_dim, hid_dim))
        self.mesh2mesh = nn.Sequential(*processor_layers)
        self.mesh2grid = Mesh2GridGNN(hid_dim, hid_dim, hid_dim, hid_dim)
